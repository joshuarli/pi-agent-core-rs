...
from .replay import ReplayError, replay_artifact
...
def parser() -> argparse.ArgumentParser:
    ...
    fast = sub.add_parser("fast", help="run provider-free strict core parity fixtures")
    ...
    replay = sub.add_parser("replay", help="verify a recorded core report without adapters, credentials, or network")
    replay.add_argument("artifact", type=Path, help="a core case report.json artifact")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "fast":
            ...
            print(
                f"quality fast: {summary['matches']}/{summary['case_count']} exact matches; "
        ...
        if args.command == "replay":
            print(json.dumps(replay_artifact(args.artifact), indent=2, sort_keys=True))
            return 0
        ...
        if args.command == "full":
            ...
            print(
                f"quality full: core {core_summary['matches']}/{core_summary['case_count']} exact; "
        ...
        print(f"quality replay error: {error}", file=sys.stderr)
        return 2
    return 2
