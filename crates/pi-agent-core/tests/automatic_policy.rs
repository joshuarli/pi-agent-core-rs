...
use pi_agent_core::state::{AgentToolCall, SerializedJson, StopReason, ToolCallId, Usage};
use pi_agent_core::tool::{
    AgentTool, ToolCall, ToolContext, ToolFuture, ToolResult, ToolUpdateSink,
};
use pi_agent_core::{Agent, AgentMessage, CoreError};
    ...
    smol::block_on(async {
        ...
        assert!(matches!(
            run.events().iter().find_map(|event| match &event.kind {
                pi_agent_core::AgentEventKind::AutomaticCompactionEnd { outcome, .. } => Some(outcome),
                _ => None,
            }),
            Some(pi_agent_core::AutomaticCompactionOutcome::Cancelled)
        ));
        Ok::<(), CoreError>(())
    })
