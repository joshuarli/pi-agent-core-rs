//! Dependency-free integration matrices for the public V0 contracts.
//!
//! These tests intentionally live outside `pi-agent-core`'s unit-test module.  They exercise
...
use pi_agent_core::profile::{PiDefaultCodingProfile, ProfileSpec};
...
fn schema() -> pi_agent_protocol::JsonValue {
...
struct ScriptedProvider {
    streams: Mutex<VecDeque<Result<ModelStream, pi_agent_core::error::SchedulerError>>>,
}

impl ScriptedProvider {
    fn new(
        streams: impl IntoIterator<Item = Result<ModelStream, pi_agent_core::error::SchedulerError>>,
    ) -> Self {
        Self {
...
struct YieldingResult {
    remaining: u8,
    result: Option<Result<ToolResult, pi_agent_core::error::ToolError>>,
}

impl Future for YieldingResult {
    type Output = Result<ToolResult, pi_agent_core::error::ToolError>;
...
fn normal_stream(text: &str) -> Result<ModelStream, pi_agent_core::error::SchedulerError> {
    Ok(ModelStream {
        events: vec![
...
fn terminal_cleanup_matrix_allows_reuse_after_drop_success_and_model_error() {
    smol::block_on(async {
        ...
        assert_eq!(
            error,
            pi_agent_core::error::CoreError::ModelError {
                message: "fixture model failure".into()
            }
        );
...
fn assert_lifecycle_balance(events: &[pi_agent_core::event::AgentEvent]) {
