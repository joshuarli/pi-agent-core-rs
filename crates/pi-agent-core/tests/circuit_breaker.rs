...
use pi_agent_core::state::{AgentToolCall, SerializedJson, StopReason, ToolCallId};
...
use pi_agent_core::{Agent, AgentEventKind, AgentMessage, CoreError};
...
impl AgentTool for FailureTool {
    ...
    fn schema(&self) -> &pi_agent_protocol::JsonValue {
        ...
        SCHEMA.get_or_init(|| {
            pi_agent_protocol::JsonValue::parse(r#"{"type":"object","required":["text"]}"#).unwrap()
        })
    }
