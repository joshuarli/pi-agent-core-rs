...
//! [`pi_agent_core::tool::AgentTool`] contract. A handler is evaluated in a
...
use pi_agent_core::scheduler::{CancellationToken, CancellationWait};
use pi_agent_core::state::{SerializedJson, ToolCallId, Usage};
...
const HANDLER_CHUNK_NAME: &str = "pi-agent-tool-handler.luau";
...
fn validate_limits(limits: HandlerLimits) -> Result<(), ToolHandlerInitError> {
    for (field, value) in [
...
fn validate_spec(spec: &ToolHandlerSpec) -> Result<(), ToolHandlerInitError> {
    for (field, value) in [
