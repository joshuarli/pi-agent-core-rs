use pi_agent_core::hooks::BeforeToolCall;
use pi_agent_core::state::{SerializedJson, ToolCallId};
use pi_agent_core::tool::ToolCall;
...
fn ambient_process_file_module_and_debug_globals_are_not_available() {
    let policy = LuaPolicy::load(
        r#"
            local require_ok = pcall(function() return require("pi_agent_core_missing") end)
            local loadfile_ok = pcall(function()
                return loadfile("/__pi_agent_core_missing_file__")
            end)
            local dofile_ok = pcall(function()
                return dofile("/__pi_agent_core_missing_file__")
            end)
            return {
...
fn sandbox_rejects_global_table_mutation() {
    let error = match LuaPolicy::load(
        r#"
            _G.pi_agent_core_hardening_probe = true
